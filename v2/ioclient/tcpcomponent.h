...
SOCKET newServer(unsigned short port, bool reuseAddr = false, int backlog = 1024) {
    if (!initWSA()) return INVALID_SOCKET;
    ...
    char portStr[6];
    sprintf_s(portStr, "%u", port);
    int err = getaddrinfo(nullptr, portStr, &hints, &res);
    if (err != 0) {
...
bool resolveHost(const char* host, unsigned short port, sockaddr_in6& outAddr) {
    addrinfo hints{}, * res = nullptr;
    hints.ai_family = AF_UNSPEC;          // IPv4 или IPv6
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_protocol = IPPROTO_TCP;
    ...
    for (addrinfo* p = res; p != nullptr; p = p->ai_next) {
        if (p->ai_family == AF_INET) {
            //std::cout << "resolved to v4\n";
            sockaddr_in* v4 = (sockaddr_in*)p->ai_addr;
            outAddr.sin6_family = AF_INET6;
            outAddr.sin6_port = v4->sin_port;
            outAddr.sin6_addr = in6addr_any;
            memcpy(&outAddr.sin6_addr.u.Byte[12], &v4->sin_addr, 4); // последние 4 байта
            outAddr.sin6_addr.u.Word[5] = 0xffff;                    // ::ffff:IPv4
            ok = true;
            break;
        }
        else if (p->ai_family == AF_INET6) {
            memcpy(&outAddr, p->ai_addr, sizeof(sockaddr_in6));
            ok = true;
            break;
        }
    }
